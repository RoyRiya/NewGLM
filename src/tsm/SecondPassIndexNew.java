...
class GLMTermWeight{
    ...
    public GLMTermWeight(String term1){
    	this.term=term1;
    }

}

public class SecondPassIndexNew {
    ...
    public  SecondPassIndexNew(String propFile) throws FileNotFoundException, IOException
    {
        prop = new Properties();
    ...
    List<GLMTermWeight> computeNormalizedTf(Terms terms, int docLen) throws IOException
    {
        BytesRef term;
        ArrayList<GLMTermWeight> tsim = new ArrayList<>();
    	TermsEnum termsEnum = terms.iterator(null);
        ...
        while ((term = termsEnum.next()) != null) { 
            ...
            while (docsEnum.nextDoc() != DocIdSetIterator.NO_MORE_DOCS) {
                ...
                GLMTermWeight tobj = new GLMTermWeight(termStr);
                tobj.ntf_d = ntf;
                tsim.add(tobj);
            }
        }
    ...
    List<GLMTermWeight> computeDocTransformProbs(List<GLMTermWeight> tstats)
    {
        ...
        for (i = 0; i < len; i++) {
            GLMTermWeight tf_i = tstats.get(i);
            totalPairwiseSim = 0;
            for (j = 0; j < len; j++)
            {
                if (i==j)
                    continue;
                GLMTermWeight tf_j = tstats.get(j);
  ...
  public List<GLMTermWeight> AveragingWeights(HashMap<String,Integer> hash , List<GLMTermWeight> tstats)
  {
      int len=tstats.size();
      List<GLMTermWeight> expandedTransformedList = new ArrayList<GLMTermWeight>();
      for(String st : hash.keySet())
      {
          ...
          else // we have to take the average of all the prpbability
          {
              ...
              for(int i=0;i<len;i++)
              {
                ...
                GLMTermWeight gm = new GLMTermWeight(st); 
                gm.ntf_d=term_prob;
  ...
  public HashMap getUniqueTerms(List<GLMTermWeight> tstats) // This method returns the unique_terms of the document along with the frequency 
  {
  ...
  public  List<GLMTermWeight> computeCollTransformProb(List<GLMTermWeight> tstats, int K, float thresh) throws IOException 
  {
        List<GLMTermWeight> expandedAndTransformedTermStats = new ArrayList<GLMTermWeight>(tstats); // copy the objects,this list is to grow with new terms
	//List<GLMTermWeight> mapOfExpandedTerms = new ArrayList<GLMTermWeight>(); // need to accumulate weights
	int len = tstats.size(),i;
        ...
        for (i = 0; i < len; i++)
        {   
            GLMTermWeight ts_i = tstats.get(i);
            ...
            for (WordVec nn : nn_tf_i)
            {
               if(check_presence_in_collection(nn.word)!=0)
               {
                   if(unique_terms.get(nn.word)==null)
                    ...
                    GLMTermWeight tm= new GLMTermWeight(nn.word); // expanded terms will not have prob_in_doc or ntf_d
                    tm.prob_in_doc=0;
                    tm.ntf_d=0;
		...
		// add each element to the transformed array.
                ...
                List<GLMTermWeight> new_transformed_list=AveragingWeights(unique_terms,expandedAndTransformedTermStats); 
  ...
  public void expandDoc(IndexReader reader, int docId) throws IOException, Exception
  {
	...
	int docLen = getDocLen(terms); // compute doc length
        if(docLen==0)
            return;
	List<GLMTermWeight> tstats = computeNormalizedTf(terms, docLen); // normalize tfs

	List<GLMTermWeight> docTransformWeights = computeDocTransformProbs(tstats); // compute GLM doc transformation

		// compute GLM coll transformation (array size expected to be higher due to expanded terms)
	List<GLMTermWeight> collTransformWeights = computeCollTransformProb(docTransformWeights, K, thresh);
  ...
  public void writeDocToIndex(IndexWriter writer,Document doc,List<GLMTermWeight> transformed_list) throws Exception
  {
  ...
  public static void main(String[] args) throws IOException, Exception {
        if (args.length == 0) 
        {
            ...
            SecondPassIndexNew indexer = new SecondPassIndexNew(args[0]);
            indexer.expandIndex();                
